...
| **OBL-30** | Wire `scripts/verify-workflow-structure.py` into a CI workflow so a silently deleted CI guard actually fails a build. | **This is `OBL-28` reopened**; its 2026-07-28 closure was retracted on 2026-07-29 and the reasoning is in the Closed table. The verifier is real and good — 10 of 10 mutations caught — but `grep -rn "verify-workflow-structure" .github/` returns nothing, so it runs nowhere. It is a control that exists and does not execute. **Adding an enforcement gate to CI is a Founder decision, not an engineering one**, which is why this is recorded rather than done. Recommendation, explicitly labelled as such: add it to `lint.yml`'s `actions` job, which already validates workflow files and already triggers on both long-lived branches. Note `CI-03` and `CI-10` in `REVIEW_FINDINGS_HANDOFF.md` §3 report gaps in the verifier itself; wiring it in without those fixes enforces a check that misses 5 change classes. | The verifier runs in a workflow on push and pull request, and a deliberately deleted guard turns that workflow red. |
| **OBL-32** | Untrack `scripts/__pycache__/test-verify-workflow-structure.cpython-314.pyc`. | Python bytecode committed in `c8c4c38`. Build output does not belong in version control: it is architecture- and interpreter-version-specific (`cpython-314`), it changes on every run, and it will produce spurious diffs on any machine with a different Python. **The `.gitignore` half is done** — `__pycache__/` and `*.py[cod]` rules added 2026-07-29, verified with `git check-ignore -v` against a new path. Ignoring does not untrack: `git ls-files --error-unmatch` still finds the file, so `git rm --cached` is required and was deliberately not run (a shared working tree with concurrent writers is not the place for index-mutating commands). | `git ls-files scripts/__pycache__/` returns nothing. |
...
| **OBL-33** | Correct two in-code records that contradict verified repository state. | Both are single-line documentation defects inside source files, found 2026-07-29 by a pass whose file scope did not include them. Neither affects behaviour; both mislead the next reader. **(a) `eslint.config.mjs:28-30`** says the tsc exclusion *"remains open as OBL-08"*. OBL-08 is in the Closed table of this register, and `tsconfig.json:33` carries the explicit `"exclude": ["node_modules", ".semgrep/fixtures"]` that closed it. Fix: replace *"is still incidental… and remains open as OBL-08"* with a statement that `tsconfig.json` now excludes the path explicitly. **(b) `e2e/mission-control-viewport.spec.ts:120-123`** calls the no-horizontal-scroll assertion *"the assertion that distinguishes 'renders on mobile' from 'usable on mobile'."* Under the e2e build `proxy.ts` 403s the whole Dev HQ surface, and `MissionControlOverview.tsx:94` returns a loading placeholder whenever the state snapshot is absent — so the assertion measures a five-box skeleton, not the operational view. Fix: say it checks the shell does not overflow, and point at `OBL-11` for the gap. | Both comments state what is true, and neither cites a closed obligation as open. |
