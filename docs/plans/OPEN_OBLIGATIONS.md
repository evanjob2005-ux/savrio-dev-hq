...
| **OBL-22** (A-04) | Give the overrides an exit path. Add a Renovate `packageRule` with `matchDepTypes: ["overrides"]` grouping each override with its motivating parent, plus a check reporting when an override's floor is already satisfied by every declared consumer. | Renovate detects `overrides` and will keep *raising* floors, but nothing ever proposes *removing* one. The removal condition is documented and has no trigger, so the default outcome is permanent. Degrades at roughly a dozen entries, when the annotation stops being read. | A Renovate dry run showing an override grouped with its parent, and the redundancy check reporting `brace-expansion` and `engine.io` as removable today. |
| **OBL-23** (A-06) | Run the whole-tree `npm audit` on pull requests, not only on push. | `dependencies.yml:115` skips the audit job on PRs, so the proof that closed OBL-03 executes only *after* merge. `dependency-review` gates newly-introduced vulnerabilities in the diff and never re-asserts the whole-tree property — the stated reason for the exclusion is inaccurate. Roadmap §9 requires the proof to run on the candidate. | A PR check on a branch showing `npm audit passed: no high or critical vulnerabilities`. |
| **OBL-24** (A-09) | Add audit re-proof to OBL-09's completion condition, and confirm the `npm-audit` job's npm version. | Regenerating the lockfile means re-resolving under a different resolver, and overrides interact with peer resolution — the exact mechanism the npm pin exists to work around. Also: the pin is applied in `ci.yml` and `frontend-tests.yml` but **not** in `dependencies.yml`. Whether npm 10.9.4 can build an ideal tree from this lockfile is unverified. | OBL-09 states the audit re-proof, and the audit job either carries the pin or records that it is unaffected. |
