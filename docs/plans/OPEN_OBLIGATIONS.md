...
| **OBL-03** | Retire the 20 accepted advisories in `.github/audit-exceptions.json` as upstream fixes land. **All expire 2026-10-26.** | Triaged 2026-07-28: `npm audit` reports 40 advisories, of which 20 are high/critical and **7 reach production**. Every one offers only a MAJOR DOWNGRADE as its fix — `npm audit fix --force` would take Next 16.2.11 to 9.3.3 and Trigger.dev 4.5.7 to 1.0.8. The fix is upstream, not local. The critical (`tar`) is dev-scope and does not ship. | Each entry removed as its advisory clears, with the audit gate passing on a shrinking exception list. |
