...
| **OBL-28** | Build a sound workflow-structure verifier, then wire it into CI with `fetch-depth: 0`. | The cycle-2 defect — two `if:` guards silently deleted from CI gate steps — passed YAML validation, step-name resolution, and all 20 gates. Nothing in this repository can currently detect that class. A first attempt was written and **removed unmerged**: independent review ran ten adversarial mutations against it and it missed seven, including an altered `run:`, a step reorder, a changed action SHA, and deletion of `id: detect` — which would silently disable all six guarded steps in `application-validation` while reporting success. It also printed `structure preserved` and exited 0 when the base ref did not resolve, which is the default state under `actions/checkout` at `fetch-depth: 1`. | A verifier that compares every step field plus job identity, hard-fails when the base ref is missing, and ships with the ten-mutation matrix checked in as negative controls — the same positive-control standard `security.yml` already demands of Semgrep. |
