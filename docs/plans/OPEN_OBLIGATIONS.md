...
| ID | Obligation | Why it matters | Proof of completion |
|---|---|---|---|
| **OBL-09** (CR-12) | Regenerate and validate `package-lock.json`. | `npm@11.16.0` is pinned in **four steps across three workflows** to work around a lockfile that older npm cannot install. (Originally recorded as two workflows; the count grew as the audit job was activated and CI moved to Node 24.) It is a pin, not a repair, it is now load-bearing in two places, and it will drift. | Both pins removed and `npm ci` succeeding on the Node-bundled npm. |
